...
const createParser = (_parsers, [key, val]) => ({
	..._parsers,
	[key]: parser(val, key.slice(1)),
});

const parsers = Object.entries(RegExpPatterns).reduce(createParser, {});
const arrayParsers = Object.entries(ArrayPatterns).reduce(createParser, {});
...
function parseArray(state, match) {
	let result = {};

	while (state.data.length && match && result?.type != 'CloseArray') {
		result = arrayParsers._CloseArray(state.data);
		if (result.value) {
...
const verifyResults = verify(' true ] ');
